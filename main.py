...
def sanitize_selection(values: List[str]) -> List[str]:
    seen = []
    all_selected = False
    for value in values:
        if value == "All":
            ...
            seen.append(value)
    return ["All"] if all_selected else seen
    ...
    st.session_state.messages.append(
        {
            ...
            "caption": "[All]을 선택하면 해당 그룹의 모든 옵션이 포함됩니다.",
...
def render_filter_controls(_: str | None = None) -> None:
    for cfg in CATEGORY_CONFIG:
        ...
        initial_value = sanitize_selection(list(st.session_state.get(state_key, [])))
        st.session_state[state_key] = initial_value

        def _on_change(key: str = state_key) -> None:
            updated = sanitize_selection(list(st.session_state.get(key, [])))
            if updated != st.session_state[key]:
                st.session_state[key] = updated

        st.multiselect(
            label=f"{cfg['label']} 조건",
            options=st.session_state.filter_options[cfg["key"]],
            key=state_key,
...
def render_system_prompt() -> None:
    with st.chat_message("assistant"):
        ...
        st.caption("[All]을 선택하면 해당 그룹의 모든 옵션이 포함됩니다.")
...
def get_active_filters() -> Dict[str, List[str]]:
    ...
    for cfg in CATEGORY_CONFIG:
        ...
        if "All" in selection or not selection:
            active[key] = options[1:]
        else:
            filtered = [opt for opt in selection if opt in options and opt != "All"]
            active[key] = filtered
    return active
